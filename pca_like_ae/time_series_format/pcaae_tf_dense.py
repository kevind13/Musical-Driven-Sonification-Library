...
X_train = np.array(scipy.io.loadmat('timeseries_midi_dataset_with_transpose.mat')['train_data'], dtype=np.float32)
...
if autoencoder:
    AE_E = Encoder(latent_space_dimension=latent_space_dimension)
    AE_D = Decoder()
    ...
    with open('pcaae_models/dense/loss_transpose.pickle', 'wb') as handle:
        pickle.dump(history, handle, protocol=pickle.HIGHEST_PROTOCOL)
    

    for model in range(latent_space_dimension):
        PCAAE_E[model].save(f'pcaae_models/dense/encoder_transpose_{model+1}',save_format='tf')
        PCAAE_D[model].save(f'pcaae_models/dense/decoder_transpose_{model+1}',save_format='tf')
