...
if autoencoder:
    ...
    with open('pcaae_models/conv/loss_conv.pickle', 'wb') as handle:
        pickle.dump(history, handle, protocol=pickle.HIGHEST_PROTOCOL)
    

    for model in range(latent_space_dimension):
        PCAAE_E[model].save(f'pcaae_models/conv/encoder_conv_{model+1}',save_format='tf')
        PCAAE_D[model].save(f'pcaae_models/conv/decoder_conv_{model+1}',save_format='tf')
