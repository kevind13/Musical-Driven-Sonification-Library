...
X_train1 = np.array(scipy.io.loadmat('timeseries_midi_dataset.mat')['train_data'], dtype=np.uint8)
...
X_train, X_test =train_test_split(X_train1, test_size=0.2)
...
if autoencoder:
    ...
    with open('pcaae_models/loss_all.pickle', 'wb') as handle:
        pickle.dump(history, handle, protocol=pickle.HIGHEST_PROTOCOL)
    

    for model in range(latent_space_dimension):
        PCAAE_E[model].save(f'pcaae_models/encoder_all_{model+1}',save_format='tf')
        PCAAE_D[model].save(f'pcaae_models/decoder_all_{model+1}',save_format='tf')
